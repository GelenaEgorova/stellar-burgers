...
import { ordersSlice, initialState, getOrders, getFeeds, getOrderByNumber, resetOrder, postOrderBurger } from '../ordersSlice';
...
describe('Тест для ordersSlice', () => {
    ...
    test('Тест для успешной обработки (getOrders.fulfilled)', () => {
        ...
        const state = ordersSlice.reducer(initialState, { type: getOrders.fulfilled.type, payload: ordersPayload });
    ...
    test('Тест для успешной обработки (getFeeds)', () => {
        ...
        const state = ordersSlice.reducer(initialState, { type: getFeeds.fulfilled.type, payload: OrderState });
        expect(state).toEqual({
            ...initialState,
            isLoading: false,
    ...
    test('Тест для статуса ожидания (postOrderBurger)', () => {
        const state = ordersSlice.reducer(initialState, { type: postOrderBurger.pending.type });
        expect(state.status).toBe(RequestStatus.Loading);
    });
    ...
    test('Тест для успешной обработки (postOrderBurger)', () => {
        ...
        const state = ordersSlice.reducer(initialState, { type: postOrderBurger.fulfilled.type, payload: orderPayload });
        expect(state).toEqual({
            ...initialState,
            isLoading: false,
    ...
    test('Тест для статуса ожидания (getOrderByNumber)', () => {
        const state = ordersSlice.reducer(initialState, { type: getOrderByNumber.pending.type });
        expect(state.status).toBe(RequestStatus.Loading);
    });
    ...
    test('Тест для успешной обработки (getOrderByNumber)', () => {
        ...
        const state = ordersSlice.reducer(initialState, { type: getOrderByNumber.fulfilled.type, payload: orderModalPayload });
        expect(state).toEqual({
            ...initialState,
            isLoading: false,
